...
finalHands: .space 30	# Areas where the player's final hand is stored
AIHand:	.ascii "\nAI's hand: "
AIFinalHand: .space 30		# Areas where the AI's final hand is stored

playerWins: .asciiz "You Won!"
AIWins:	.asciiz "The computer won."

exitPrompt: .asciiz "Would you like to exit the program?\n If you hit cancel we are going to guess you want have another hand."

# This is where the arrays for the hand and eventually the exchanged cards and the suits are stored
hand: .word 0 : 10
...
beq $a1, -1, invalidChoice	#If the input character/s changed the status to a non-OK status, branches to invalidChoice for it
beq $a1, -2, exiting	# If the user hits cancle sees if the user decides that they want to exit by hitting cancel
beq $a1, -3, invalidChoice	# If the user hit OK but there is not input
...
beq $a1, -1, invalidChoice	#If the input character/s changed the status to a non-OK status, branches to invalidChoice for it
beq $a1, -2, exiting	# Branches to the exiting subroutine to set a new hand or 
beq $a1, -3, invalidChoice	# If OK was selected but no information was given
...
ending:
beqz $t8, secondEval	# If there is a zero, basically no value, in t8 it runs the program another time to evaluate AI's hand
li $v0, 59	# Preps to display a message string box, that displays two different strings
la $a0, player	# Loads the player's hand
la $a1, AIHand	# Loads the AI's hand
syscall	# Displays the mesage string box
bgt $t8, $t9, playerWin	# If the player hand's value is greater than the AI's
blt $t8, $t9, AIWin	# If the AI hand's value is greater than the player's
beq $t8, $t9, tieBreaker	# If both the player's and AI's hands were of the same value, same kinds of hands goes to a tie breaker

# This is if the players has won the hand
playerWin:
jal findPlayerHand	# Finds the string for the player's hand, what kind of hand
jal findAIHand	# Finds the string for the AI's hand
syscall	# Displays another message string box to display both types of hands, player's and AI's
li $v0, 55	# Preps to display a message box
la $a0, playerWins	# Loads the string if the player has won the hand
syscall	# Displays the message box
b exiting	# Branches to exit

# This is if the AI won the round. The only difference between the previous one is that the final string is a little different
AIWin:
jal findPlayerHand
jal findAIHand
syscall
...
# This decides who the winner is if there was hand type tie
tieBreaker:
...
exiting:
li $v0, 50	# Preps to launch a comfirmation box with a yes no prompts
la $a0, exitPrompt	# Loads the message for the confirmation box
syscall	# Launches the confirmation box
beqz $a0, exit	# If the player chooses yes then the program branches to the exit subroutine
beq $a0, 2, exit	# If the player chooss cancel then we assume they do want to exit
# The following rezeroes all of the temporary counters that are used throughout
# The arrays and buffers will be overwritten when they are reloaded and start from the beginning
li $t0, 0
li $t1, 0
li $t2, 0
li $t3, 0
...
li $t8, 0
li $t9, 0
b deal	# Branches back to the deal loop
...
# This is the area where the various hands strings for the player are displayed
# All are basically the same but for different types of hands so different strings in the .data section
findPlayerHand:
beq $t8, 1, highCardp	# If the value in t8 is 1 then it branches to the highcardp section
# The rest are the same for each other type of hand possible
beq $t8, 2, pairp
beq $t8, 3, twoPairp
...
# These labels are only to load the appropriate string, all of them are the same for the first, except as above different strings
highCardp:
la $a0, HighCard	# Loads the highcard string for players in a0
jr $ra	# Then it jumps back to the appropriate area in the code
pairp:
la $a0, Pair
jr $ra
twoPairp:
...
# This section finds the AI's hand and prepares to display it in the message string box, as above the only differences are the 
# strings being loaded
findAIHand:
beq $t9, 1, highCardAI2	# If the AI's hand value is of a highcard type
beq $t9, 2, pairAI2
beq $t9, 3, twoPairAI2
...
# Loads the various types of string and the string for the AI's hand
highCardAI2:	
la $a1, HighCardAI	# Loads the high card string for the AI
jr $ra	# Jumps back to the appropriate area in the code
# The same format from above is for all below except the strings differ for each type of hand
pairAI2:
la $a1, PairAI
jr $ra
twoPairAI2:
