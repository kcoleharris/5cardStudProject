...
handType: .word 0:2	#holds the type of hand for each player
highValue: .word 0:2	#holds the highest value of hand type
...
la $s4,handType  #create array to hold the type of hand (value held in $t3)
la $s5 highValue  #create array to hold the highest value in the hand type (held in $t2)
...
#STORE VALUE IN HIGH VALUE ARRAY (VALUE FOR PAIR)
#CHECK TO SEE IF LAST CHECKED WAS GREATER THAN EXISTING VALUE
lw $t8,0($s5) #load the last value placed in highValue into $t8
#LOAD LAST CHECKED ELEMENT INTO ARRAY
ble $t2,$t8,continueIsPair #if the current card being checked is less than or equal to the last value placed in highValue, take no action
sw $t2,0($s5) #else, make the current card the new highValue for the player on a high card hand
continueIsPair:
...
#STORE VALUE IN HIGH VALUE ARRAY (VALUE FOR BOTH PAIR, THREE OF A KIND, AND FOUR OF A KIND)
#CHECK TO SEE IF LAST CHECKED WAS GREATER THAN EXISTING VALUE
lw $t8,0($s5) #load the last value placed in highValue into $t8
#LOAD LAST CHECKED ELEMENT INTO ARRAY
ble $t2,$t8,continueTwoP #if the current card being checked is less than or equal to the last value placed in highValue, take no action
sw $t2,0($s5) #else, make the current card the new highValue for the player on a high card hand
continueTwoP:
...
sw $t3,0($s4)	#store hand type in position for current player
addi $s4,$s4,4	#increment to next position in handType array for next player hand
...
sw $t3,0($s4)	#store hand type in position for current player
addi $s4,$s4,4	#increment to next position in handType array for next player hand
b exit #added to remind us to add branch to highestHand here
...
lw $t7,0($s4) #load player1 hand type
lw $t8,4($s4) #load player2 hand type
bgt $t7,$t8,player1Win	#if $t7 > $t8 then player one had a better hand
blt $t7,$t8,player2Win  #if $t7 < $t8 then player two had a better hand
beq $t7,$t8,equalHands	#if there was a tie branch to the equalHands section

	#handles hand type ties (finds highest face value of cards)
	equalHands:beqz $t7, tieHand #if highCard then branch to tieHand
		   beq $t7,1,tiePair #if pair then branch to tiePair
		   beq $t7,2,tiePair #if twoPair then branch to tiePair
		   beq $t7,3,tiePair #if threePair then branch to tiePair
		   beq $t7,5,tiePair #if straight then branch to tiePair
		   beq $t7,6,tieHand #if flush then branch to tieHand
		   beq $t7,7,tieHand #if fullHouse then branch to tieHand
		   beq $t7,8,tieHand #if fourKind then branch to tieHand
		   beq $t7,9,tieHand #if straightFlush then branch to tieHand
		   beq $t7,10,tieHand #if royalFlush then branch to tieHand
		   
	#COMPARE BASED ON HIGHEST VALUE
	
	# TIE HAND -- finds the highest value in each players hand, this would be the deciding factor (each hand needs to be sorted again)
	tieHand:li $t6,0 #create loop counter for hand sorting
	tieHand2: bgt $t6,4 continueTieHand
		lw $t0, 0($s0)	#Loads the face value from the array
		lw $t1, 4($s0)	#Loads the next face value from the array
		slt $t8,$t0,$t1
		beqz $t8, newHigh
		beq $t8,1,newHigh2
			newHigh:add $t7,$0,$t0 	#store new high value in $t7
				b tieHandLoopBack
			newHigh2: add $t7,$0,$t1 #store new high value in $t7
				 #goes directly to tieHandLoopBack
		tieHandloopBack:addi $t6,$t6,1 #increase loop counter 
				b tieHand2 #repeats tieHand to sort through whole hand (skip recreating loop counter)
		continueTieHand: sw $t7,0($s5) #store highest value into highValue array
				#addi $s5,$s5,4 #increase to next position for next player's high value to be added
		#AFTER BOTH HANDS HAVE BEEN STORED IN $S5
		#COMPARE VALUES IN $S5 (HIGHEST VALUE IN RESPECTIVE PLAYER'S HANDS) TO DECLARE WINNER OF ROUND 
		lw $t7,0($s5) #load player1 high value
		lw $t8,4($s5) #load player2 high value
		bgt $t7,$t8,player1Win #player 1 card value was highest
		blt $t7,$t8,player2Win #player 2 card value was highest
		beq $t7,$t8,tieRank #both card values were the same (BREAK TIE BASED ON SUIT VALUE)
		#BRANCH BACK TO EXIT
	  
	# TIE PAIR -- finds the highest value stored in the highestValue array (the stored $t2 values from the evaluation stage)
	tiePair: lw $t7,0($s5) #load the player 1 value for $t2 (highest pair value) into $t7
		 lw $t8,4($s5) #load the player 2 highest pair value into $t8
		 bgt $t7,$t8,player1Win	#if $t7 > $t8 then player one had a better hand
		 blt $t7,$t8,player2Win  #if $t7 < $t8 then player two had a better hand
		 beq $t7,$t8,tieRank	#if there was a tie branch to the equalHands section (decide based on suit) (apolicable for pairs and twoPairs)
		 
		 
			tieRank: #FIND A WAY TO TRACK THE SUIT OF HIGHEST RANK VALUE
				#BRANCH BACK TO EXIT
		
	#HANDLES CONDITIONS IF PLAYER 1 WINS	
	#will set the stats for the player 1 to start the next round
	#player1Win: li $s6,5 #greatest index in hand array player 1 can choose
	#	    li $s7,1 #lowest index in hand array player 1 can choose 
	#	    j startNextPlayer
	
	#HANDLES CONDITIONS IF PLAYER 2 WINS
	#will set the stats for player 2 to start the next round
	#player2Win:li $s6,10 #greatest index in hand array player 2 can choose
