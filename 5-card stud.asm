...
#This part of the code and the deal loop that runs for all the cards for the hand
	la $a2, hand	#Loads the size of the hand into the array
...
outerDealLoop:
	...
	add $a2, $a2, $t2	#Readjusts to where the last random number has been rejected
...
dealLoop:
	...
	add $a2, $a2, $t2	#Adds the readjust ment
	...
	sw $t1, 0($a2)	#If it makes it through the innerDealLoop it saves the number to the array
	addi $a2, $a2, 4	#Adjusts the array to the next index
	addi $t0, $t0, 1	#Adds 1 to $t0 to track the loop
	bltu $t0, 5, dealLoop	#Branches if $t0 is less than 5, to dealLoop
	
addi $a2, $a2, -20	#Adjusts the entire array again to have it at the first index for evaluation
...
addi $a2, $a2, -20	#Needed to readjust where the array is currently indexing at
...
card:
	lw $t1, 0($a2)	#Loads the next integer to be check from $a2 to $t1
	...
	addi $a2, $a2, 4	#Increments to the next index in the array
